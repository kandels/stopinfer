...
def optimize_cr(campaign = campaign, background='ttbar', s_point='TT_bWN1_650_560', region='smalldm1b'):
    ...
    Strategy - Scan signal score from 0 - low bound of SR. Identiy the range where 0.05 < s/b < 0.1 --> VR. Range s/b < 0.05 --> CR.
    """
    ...
    VR_UPPER_BOUND = sr_low_bins[f'{region}{campaign}']
    ...
    step = 0.01
    scan_range = np.arange(VR_UPPER_BOUND - step, 0, -step)
    vr_lower_bound = None
    last_valid_bound = None
    found_vr_range = False
    
    for lower_bound in scan_range:
        s = sig_df[(sig_df[score_sig] >= lower_bound) & (sig_df[score_sig] < VR_UPPER_BOUND)]['yield'].sum()
        b = mc[(mc[score_sig] >= lower_bound) & (mc[score_sig] < VR_UPPER_BOUND)]['yield'].sum()
        ...
        print(f'In bin [{lower_bound:.3f}, {VR_UPPER_BOUND:.3f}] sig = {s:.3f} bkg = {b:.3f} s/b = {s_b_ratio:.4f}')
        
        # Check if this bin is in the VR range
        #if 0.05 < s_b_ratio < 0.10:
        #if 0.05 < s_b_ratio:
        if 0.05 < s_b_ratio < 0.1:
            last_valid_bound = lower_bound  # Keep track of the last valid VR bin
            found_vr_range = True
        elif found_vr_range:
            # We've found VR range before and now we're outside it
            vr_lower_bound = last_valid_bound
            print('-' * 50)
            print(f'VR range ended. Last valid VR bin was at {last_valid_bound:.3f}')
            print(f'Current bin [{lower_bound:.3f}, {VR_UPPER_BOUND:.3f}] has s/b = {s_b_ratio:.4f} (outside VR range)')
            break
        # If we haven't found VR range yet, continue scanning
    
    if vr_lower_bound is None:
        if found_vr_range:
            vr_lower_bound = last_valid_bound
        else:
            print("No VR range found - all bins have s/b outside 0.05-0.1 range")
            vr_lower_bound = "Not found"
        
    print('=' * 50)
    print(f'VR Signal score bounds for {campaign} {background} in region {region} are - [{vr_lower_bound}, {VR_UPPER_BOUND}]')
    print('=' * 50)
...
optimize_cr(campaign=campaign, background='VV', s_point='TT_bWN1_550_460', region='smalldm0b')
